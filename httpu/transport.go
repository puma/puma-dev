...
func (t *Transport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if s := req.URL.Scheme; s != "http" && s != "https" && s != "httpu" {
		closeBody(req)
...
func (t *Transport) connectMethodForRequest(treq *transportRequest) (cm connectMethod, err error) {
	...
	if cm.targetScheme == "httpu" {
		cm.targetAddr = treq.URL.Host
	} else {
...
func (t *Transport) dialConn(cm connectMethod) (*persistConn, error) {
	...
	if tlsDial {
		...
		conn, err := t.dial(cm.network(), cm.addr())
		if err != nil {
			if cm.proxyURL != nil {
...
// the network type to use, unix for httpu scheme
func (cm *connectMethod) network() string {
	if cm.targetScheme == "httpu" {
		return "unix"
	} else {
		return "tcp"
...
func (pc *persistConn) writeLoop() {
	for {
		...
		case wr := <-pc.writech:
			...
			err := writeRequestFull(wr.req.Request, pc.bw, pc.isProxy, wr.req.extra, pc.waitForContinue(wr.continueCh))
			if err == nil {
				err = pc.bw.Flush()
			}
			if err != nil {
